                    ...
                    <?php else: ?>
                        ...
                        <?php foreach ($dispatches as $d): ?>
                            ...
                            <tr id="fleet-row-<?= $d['id'] ?>" data-id="<?= $d['id'] ?>" data-truck="<?= htmlspecialchars(strtolower($d['truck'])) ?>" data-dispatch-date="<?= htmlspecialchars($d['dispatch_date']) ?>" data-mileage-cost="<?= (float)convert_currency($d['mileage_cost']) ?>" data-extra-expenses="<?= (float)convert_currency($d['extra_expenses']) ?>" data-diesel="<?= (float)convert_currency($d['diesel'] ?? 0) ?>" data-unit-price="<?= (float)convert_currency($d['unit_price'] ?? 0) ?>" data-shortage-litres="<?= (int)($d['shortage_litres'] ?? 0) ?>">
                                ...
                                <!-- Diesel Fuel Cost (Owner Expense: Total) -->
                                <td class="cell-diesel" style="white-space:nowrap;font-weight:700;">
                                    ...
                                    <?php else: ?>
                                        <span class="view-val" style="font-weight:800;color:var(--amber);font-size:13.5px;">
                                            <?= format_money($dVal) ?>
                                        </span>
                                    <?php endif; ?>
                                </td>
        ...
        <form method="POST" action="<?= url('fleet/store') ?>" id="dispatchWizardForm" novalidate onsubmit="return onWizardFormSubmit(event)">
                    ...
                    <div class="form-group">
                        ...
                        <datalist id="destPresets">
                            <?php foreach ($mileageRates as $mr): ?>
                                <option value="<?= htmlspecialchars($mr['destination']) ?>"><?= htmlspecialchars($mr['destination']) ?></option>
                            <?php endforeach; ?>
                        </datalist>
                        <span style="font-size:11.5px;color:var(--text-3);display:block;margin-top:2px;">Destination of delivery</span>
                    </div>

                    <div class="form-group">
                        <label>👤 Client / Consignee Name (Optional)</label>
                        <input type="text" name="client_name" id="dispatchClientName" placeholder="e.g. TotalEnergies Uganda Ltd (or leave blank)" style="width:100%;">
                    </div>
                </div>
            ...
            <div class="wizard-step-panel" id="wizardStep3" style="display:none;">
                <div style="background:var(--card-2);border-left:3px solid var(--amber);padding:10px 14px;border-radius:8px;margin-bottom:16px;">
                    ...
                    <div style="font-size:12px;color:var(--text-3);margin-top:2px;">Enter total driver en-route mileage allowance and diesel fuel cost. Totals update manually.</div>
                </div>

                <div class="form-grid">
                    <div class="form-group">
                        <label>Mileage & En-Route Cost [USD $] *</label>
                        <input type="number" step="0.01" name="mileage_cost" id="dispMileage" placeholder="e.g. 350.00" required oninput="calcBalance()">
                        <span style="font-size:11.5px;color:var(--text-3);display:block;margin-top:2px;">Driver allowances & road tolls</span>
                    </div>

                    <div class="form-group">
                        <label>⛽ Diesel Fuel Cost (Owner Expense) [USD $]</label>
                        <input type="number" step="0.01" name="diesel" id="dispDiesel" placeholder="e.g. 450.00" oninput="calcBalance()">
                        <span style="font-size:11.5px;color:var(--text-3);display:block;margin-top:2px;">Diesel fuel catered for by owner</span>
                    </div>
...
window.updateFleetRowTotals = function(dispatchId, totals, globalAggregates = null) {
    ...
    if (row) {
        ...
        if (cellDiesel) {
            ...
            if (dVal > 0) {
                cellDiesel.innerHTML = `<span class="view-val" style="font-weight:800;color:var(--amber);">${totals.diesel_formatted || formatSystemMoney(dVal)}</span>`;
            } else {
